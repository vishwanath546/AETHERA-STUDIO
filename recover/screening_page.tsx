...
async function elevatePrompt(rawPrompt: string, sceneCount: number): Promise<string> {
  const directorPersona = `You are the inner creative mind of an elite film director — a master who has directed over 500 award-winning productions across Hollywood blockbusters, indie darlings, luxury brand commercials, music videos, and immersive documentaries. You have the visual instincts of Roger Deakins, the storytelling depth of Christopher Nolan, the emotional precision of Denis Villeneuve, the compositional genius of Wes Anderson, the raw intensity of Alejandro González Iñárritu, and the commercial magnetism of Ridley Scott.
...
export async function generateScript(prompt: string, sceneCount: number): Promise<Scene[]> {
  ...
  const systemInstruction = `You are a veteran film director and cinematographer with 30 years of experience across 500+ productions — from Hollywood blockbusters and Sundance darlings to Cannes Palme d'Or winners, Super Bowl commercials, and Netflix original series. You have collaborated with the world's finest DPs, editors, and sound designers.
  ...
  try {
    const response = await withTimeout(
      ai.models.generateContent({
        model: "gemini-2.5-pro",
        contents: `CINEMATIC VISION FROM THE CREATIVE DIRECTOR:
"""
${elevatedConcept}
"""
...
export async function generateVisual(
  ...
  try {
    return await withTimeout(
      generateVisualWithTimeout(enhancedPrompt, videoOutputPath, sceneNumber),
      VEO_POLL_TIMEOUT,
